...
# Filter by a specific cuisine
italian <- regression_set %>%
  dplyr::filter(Italian <= 15)

italian$Italian <- italian$Italian %>% # Italian = number of Italian restaurants within a region
  as.factor()

# Filter by a specific region 
#filtered_region <- regression_set %>%
#  dplyr::filter(NUTS_ID == "DEG0D") # Region with the lowest n_rest value

# Regression 
model1 <- polr(Italian ~ MOUNT_TYPE + URBN_TYPE + COAST_TYPE + area, data = italian, method = "probit")
summary(model1)

model2 <- polr(Italian ~ log(Population) + MOUNT_TYPE + URBN_TYPE + COAST_TYPE, data = italian, method = "probit")
summary(model2)

model3 <- polr(Italian ~ log(Population) + area, data = italian, method = "probit")
summary(model3)
...
S_N <- exp(theta - mean(italian$area)*lambda[2])
